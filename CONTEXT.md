...
- XML, JSON, JSON5, YAML, HTML, JavaScript, CSS, and Liquid adapters parse their formats structurally. HTML has two shapes selected by content: `html:fragment` for body-context markup and `html:document` for a complete page (doctype and `<html>`/`<head>`/`<body>` are structural nodes, so head and body edits merge independently). HTML rendering is source-faithful: every node keeps the source text it was parsed from, so a clean merge round-trips a real page byte for byte and rewrites only the nodes it changed. JavaScript is structural by top-level declaration (Esprima boundaries, verbatim slices) and CSS is structural by selector and declaration property; both are source-preserving on a clean round-trip. The mapped-text and raw adapters share an `OpaqueTextAdapter` base for genuinely unstructured text. PNG, JPEG, GIF, ICO, and `xap` adapters share a byte-safe `BinaryFormatAdapter` base (lossless base64 scalar, conflict on divergence) and implement `IBinaryFormatAdapter`.
...
- Add lossless or source-preserving formatting preservation for JSON, YAML, and JSON5 where practical (XML, JavaScript, CSS, and HTML already round-trip clean merges exactly).
