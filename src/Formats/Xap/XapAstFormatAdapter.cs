...
public sealed class XapAstFormatAdapter : IAstFormatAdapter
{
    ...
    public AstDocument Parse(string sourceText, string? sourcePath, AstSchema schema)
    {
        ...
        return new AstDocument(Format, new AstNode("$xap", new Dictionary<string, string> { ["$type"] = "text" }, sourceText), sourcePath, sourceText);
    }
